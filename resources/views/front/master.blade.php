...
<body>
            ...
            <div class="header_top-box">
                ...
                <div class="cssmenu">
                    <ul>
                        ...
                        <li><a href="" onclick="event.preventDefault(); document.getElementById('logOutForm').submit();">Log Out</a></li>
        ...
        <div class="container">
            <div class="menu_box">
                <ul class="megamenu skyblue">
                    <li class="active grid"><a class="color2" href="{{ route('/') }}">Home</a></li>
                    @foreach($categories as $key => $category)
                    <li><a class="color3" href="{{ route('category-product', ['kak' => $category->id ]) }}">{{ $category->name }}</a></li>
                    @endforeach
                    <div class="clearfix"> </div>
                </ul>
            </div>
    ...
    <script>
        function checkEmailAddress(email) {
            $.ajax({
                method: "GET",
                url: "{{ route('get-email-availability') }}",
                data: {
                    email: email
                },
                dataType: "JSON",
                success: function(response) {
                    $('#checkEmailMsg').text(response.message);
                    if (response.success == true) {
                        $('#checkEmailMsg').css('color', 'red');
                        $('#regBtn').prop('disabled', true);

                    }
                    else{
                        $('#checkEmailMsg').css('color', 'green');
